...
class String_Comparer:
    ...
    def __init__(self,inputGapCost, inputMaxCost):
        gapCost=inputGapCost
    ...
    def getSimilarity (self,string1, string2):
        ...
        if self.maxCost > (-self.gapCost):
            maxValue = maxValue * self.maxCost;
        else:
            maxValue = maxValue * -self.gapCost
        if maxValue == 0:
            return float(self.maxCost)
        else:
            return float(unNormalizedSimilarity/maxValue)

if __name__ == "__main__":
    comparer=String_Comparer(0.5,1.0)
    print(comparer.getSimilarity("Miliman", "Milliman"))
    print(comparer.getSimilarity("Nischal", "Nishal"))
    print(comparer.getSimilarity("Nischal", "Nikhil"))
    print(comparer.getSimilarity("Nischal", "Bharath"))
    print(comparer.getSimilarity("Cameron", "David Cameron"))
